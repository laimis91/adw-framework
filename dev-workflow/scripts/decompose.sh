...
ALL_NAMES=$(jq -r '.sub_tasks[].name' "$INPUT" | tr '\n' ', ' | sed 's/,$//' | sed 's/,/, /g')
...
for i in $(seq 0 $((SUB_TASK_COUNT - 1))); do
    ...
    # Build scope list
    SCOPE=$(jq -r ".sub_tasks[$i].scope // [] | .[]" "$INPUT" | sed 's/^/- /' | paste -sd$'\n' -)
    [[ -z "$SCOPE" ]] && SCOPE="- TBD"

    # Build depends_on list
    DEPENDS=$(jq -r ".sub_tasks[$i].depends_on // [] | .[]" "$INPUT" | paste -sd', ' -)
    ...
    OTHERS=$(jq -r ".sub_tasks[] | select(.name != \"$NAME\") | .name" "$INPUT" | paste -sd', ' -)
...
for i in $(seq 1 $((SUB_TASK_COUNT - 1))); do
    NAME=$(jq -r ".sub_tasks[$i].name" "$INPUT")
    NUM=$((i + 1))
    echo "   claude -p \"\$(cat $BRIEFS_DIR/sub-task-${NUM}-${NAME}.md)\" --cwd ${WORKTREES_DIR}/${NAME}"
done
echo ""
