...
Options:
  ...
  --agent NAME         Agent CLI to use: claude or codex (default: claude)
...
case "$AGENT" in
    ...
    codex)
        ...
        ;;
    *)
        fail "Unknown agent: $AGENT. Supported: claude, codex"
        ;;
esac
...
get_branch_from_brief() {
    local brief_file="$1"
    grep -oP 'Git branch:\s*\K\S+' "$brief_file" 2>/dev/null || echo ""
}

get_worktree_from_brief() {
    local brief_file="$1"
    grep -oP 'Worktree:\s*\K\S+' "$brief_file" 2>/dev/null || echo ""
}
...
ensure_worktree() {
    ...
    if $DRY_RUN; then
        ...
        git -C "$REPO" worktree add "$worktree_path" "$branch" --quiet 2>/dev/null
        if [[ $? -eq 0 ]]; then
...
run_agent() {
    ...
    if $DRY_RUN; then
        dry "$AGENT -p \"<brief content>\" --cwd $agent_cwd > $log_file 2>&1"
        return 0
    fi
...
for i in $(seq "$START_INDEX" $((${#BRIEF_FILES[@]} - 1))); do
    ...
    if $PARALLEL; then
        ...
        run_agent "$brief" "$index" "$agent_cwd" || FAILED+=("$brief")
    fi
done
