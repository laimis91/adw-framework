...
[[ -n "$INTEGRATION_BRANCH" ]] || { fail "Missing --integration-branch."; usage; }
...
record_fail()  { FAIL_COUNT=$((FAIL_COUNT + 1)); RESULTS+=("❌ $1"); fail "$1"; }
...
if git show-ref --verify --quiet "refs/heads/$INTEGRATION_BRANCH"; then
    ...
    echo ""
    fail "Cannot continue without integration branch."
    exit 1
fi
...
if [[ ${#SUB_BRANCHES[@]} -eq 0 ]]; then
    ...
    echo ""
    fail "Expected branches like ${BRANCH_PREFIX}contracts, ${BRANCH_PREFIX}sub-task-2, etc."
    exit 1
fi
